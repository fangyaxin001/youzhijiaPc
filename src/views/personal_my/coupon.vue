<template>
  <div>
    <!-- 顶部组件 -->
    <homeTop></homeTop>
    <!-- 搜索组件 -->
    <loginHead @searchName="searchName" :seatch="true"></loginHead>
    <!-- 悬浮组件 -->
    <fielxRight></fielxRight>
    <!-- 搜索结果 -->
    <searchResult
      v-if="homeIndex == 0"
      :searchValue="searchVale"
    ></searchResult>
    <!-- 个人中心 -->
    <div class="mycenter">
      <!-- 左边导航栏 -->
      <personalLeft
        :showIndex="listIndex"
        @leftIndex="leftIndex"
      ></personalLeft>
      <div class="order_right">
        <div class="order_ul">
          <ul>
            <li
              @click="listIndex = 6"
              :class="listIndex == 6 ? 'styleColer' : ''"
            >
              优惠劵
              <div v-show="listIndex == 6" class="heng"></div>
            </li>
            <li
              @click="listIndex = 7"
              :class="listIndex == 7 ? 'styleColer' : ''"
            >
              领劵中心
              <div v-show="listIndex == 7" class="heng"></div>
            </li>
          </ul>
        </div>
        <!-- 优惠劵 -->
        <div class="you_box" v-show = "listIndex == 6">
          <div class=" flex_sb coup_box">
            <div class="coup_b flex_sb"  v-for="(item,index) in 4"  :key="index" :class="index>1 ? 'bc_img' : '' ">
                <div class="left">
                    <p class="p1">全场通用优惠券</p>
                    <p class="p2">满98减5元</p>
                    <p class="p3">有效期至 2021-12-25</p>
                </div>
                <div class="right">
                    <p  class="p1"><span>￥</span>5</p>
                    <p class="p2" v-if="index<2">未使用</p>
                    <p class="p2 font_color" v-else>已使用</p>
                </div>
            </div>
          </div>
        </div>
        <!-- 领劵中心 -->
           <div class="you_box" v-show = "listIndex == 7">
          <div class=" flex_sb coup_box">
            <div class="coup_b flex_sb"  v-for="(item,index) in 4"  :key="index">
                <div class="left">
                    <p class="p1">全场通用优惠券</p>
                    <p class="p2">满98减5元</p>
                    <p class="p3">有效期至 2021-12-25</p>
                </div>
                <div class="right">
                    <p  class="p1"><span>￥</span>5</p>
                    <p class="p2">立即领取</p>
                </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <!-- 底部组件 -->
    <bottombei></bottombei>
  </div>
</template>

<script>
// @ is an alias to /src
import homeTop from "@/components/homeTop/homeTop.vue"; //顶部组件
import loginHead from "@/components/loginHead/loginHead.vue"; //搜索组件
import bottombei from "@/components/bottomTitle/bottomBei.vue"; //底部组件
import fielxRight from "@/components/homeTop/fielxRight.vue"; //悬浮组件
import searchResult from "@/components/home/searchResult.vue"; //搜索结果
import personalLeft from "@/components/personal_ponents/personal_left.vue"; //搜索结果
export default {
  name: "Home",
  components: {
    homeTop,
    loginHead,
    bottombei,
    fielxRight,
    searchResult,
    personalLeft,
  },
  data() {
    return {
      searchVale: "", //接收的搜索值
      homeIndex: 1, //控制组件
      listIndex: 6,
    };
  },
  methods: {
    // 接受搜索的结果
    searchName(e) {
      console.log(e);
      this.homeIndex = e.index;
      this.searchVale = e.value;
    },

    //  接受子组件传过来的索引
    leftIndex(e) {
      console.log(e);
      if (e > 5 && e < 8) {
        this.listIndex = e;
      } else {
        return;
      }
    },
  },
  created() {},
};
</script>
<style lang="less" scoped>
.mycenter {
  width: 1200px;
  margin: auto;
  margin-top: 25px;
  display: flex;
  justify-content: space-between;
  .order_right {
    width: 970px;
    .order_ul {
      ul {
        background-color: #fff;
        display: flex;
        padding-left: 10px;
        box-sizing: border-box;
        height: 47px;
        li {
          width: 80px;
          text-align: center;
          height: 45px;
          line-height: 45px;
          margin-left: 10px;
          color: #999999;
          cursor: pointer;
        }
        .heng {
          border-top: 3px solid #ff61a1;
          width: 40px;
          margin: auto;
        }
      }
    }
    .styleColer {
      color: #ff61a1 !important;
    }
    .you_box {
        background-color: #fff;
        margin-top: 2px;
        min-height: 500px;
      .coup_box {
          width: 780px;
          margin: auto;
          flex-wrap: wrap;
        .coup_b {
            
          width: 330px;
          margin-left: 20px;
          height: 112px;
          margin-top: 40px;
          background-image: url("../../assets/image/orgin_img.png");
          background-repeat: no-repeat;
          block-size: 100%,100%;
         .left{
                 width: 200px;
                 text-align: center;
           .p1{
               color: #333333;
               font-size: 14px;
               margin: 7px;
           }
           .p2{
               color: #333333;
               font-size: 18px;
               font-weight: Bold;
               margin: 0;
           }
           .p3{
               color: #999999;
               font-size: 14px;
               margin: 7px;
           }
             }
             .right{
                    width: 120px;
                 text-align: center;
                 .p1{
                 font-size: 32px;
                 font-weight: bold;
                 color: white;
                 margin: 10px;
                 span{
                     font-size: 18px;
                 }
                 }
                 .p2{
                   width: 72px;
                   height: 28px;
                   border-radius: 20px;
                   line-height: 28px;
                   text-align: center;
                   color: #FF2B4C;
                   background-color: #fff;
                   margin: auto;
                   font-size: 12px;
                   cursor: pointer;
                 }
             }
        }
        .bc_img{
             background-image: url("../../assets/image/gray_img.png");
        }
        .font_color{
            color: #999999 !important;
        }
      }
    }
  }
}
</style>
